...
class SamePad2d(nn.Module):
    ...
    def forward(self, input):
        ...
        # math.ceil()向上取整， math.bottom()向下取整
        out_width = math.ceil(float(in_width) / float(self.stride[0]))
        out_height = math.ceil(float(in_height) / float(self.stride[1]))
        # out = (in - kernel + 2 * pad) / stride + 1 => in = floor([(out - 1) * stride + kernel - in] / 2)
...
# FPN(C1, C2, C3, C4, C5, out_channels=256) C1-C5是ResNet的输出
class FPN(nn.Module):
    def __init__(self, C1, C2, C3, C4, C5, out_channels):
        ...
        # C5输出通道：2048 P5：2048 -> self.out_channels
        self.P5_conv1 = nn.Conv2d(2048, self.out_channels, kernel_size=1, stride=1)
        self.P5_conv2 = nn.Sequential(
            SamePad2d(kernel_size=3, stride=1),
            nn.Conv2d(self.out_channels, self.out_channels, kernel_size=3, stride=1),
        )
        # C4输出通道：1024 P4：1024 -> self.out_channels
        self.P4_conv1 =  nn.Conv2d(1024, self.out_channels, kernel_size=1, stride=1)
        self.P4_conv2 = nn.Sequential(
            SamePad2d(kernel_size=3, stride=1),
            nn.Conv2d(self.out_channels, self.out_channels, kernel_size=3, stride=1),
        )
        # C3输出通道：512 P3：512 -> self.out_channels
        self.P3_conv1 = nn.Conv2d(512, self.out_channels, kernel_size=1, stride=1)
        self.P3_conv2 = nn.Sequential(
            SamePad2d(kernel_size=3, stride=1),
            nn.Conv2d(self.out_channels, self.out_channels, kernel_size=3, stride=1),
        )
        # C2输出通道：256 P2：256 -> self.out_channels
    ...
    def forward(self, x):
        x = self.C1(x) # size: 1/2
        x = self.C2(x) # size: 1/4
        c2_out = x
        x = self.C3(x) # size: 1/8
        c3_out = x
        x = self.C4(x) # size: 1/16
        c4_out = x
        x = self.C5(x) # size: 1/32
...
class ResNet(nn.Module):
    ...
    def __init__(self, architecture, stage5=False):
        ...
        self.C1 = nn.Sequential(
            ...
            SamePad2d(kernel_size=3, stride=2), # padding
    ...
    # C1, C2, C3, C4, C5 = resnet.stages() 返回5层网络结构
    def stages(self):
    ...
    def make_layer(self, block, planes, blocks, stride=1):
        ...
        # Bottleneck: def __init__(self, inplanes, planes, stride=1, downsample=None)
        # Bottleneck的输入通道：self.inplanes 输出通道：4 * planes
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample))
        self.inplanes = planes * block.expansion
        # 添加stride = 1的层数，增加网络深度
...
def clip_boxes(boxes, window):
    #  window = np.array([0, 0, height, width]) 图像的边界
    """
...
def proposal_layer(inputs, proposal_count, nms_threshold, anchors, config=None):
    ...
    """
    # 提取proposal：Filtering is done based on anchor scores and non-max suppression to remove overlaps.
    ...
    # [batch, N, (y1, x1, y2, x2)]
    boxes = apply_box_deltas(anchors, deltas) # Get refined anchors

    # Clip to image boundaries. [batch, N, (y1, x1, y2, x2)]
    height, width = config.IMAGE_SHAPE[:2]
    window = np.array([0, 0, height, width]).astype(np.float32)
    boxes = clip_boxes(boxes, window) # 修正anchor坐标到图像范围内
...
def pyramid_roi_align(inputs, pool_size, image_shape):
    ...
    # rpn_rois: [proposal_count, 4] -> [proposal_count, 1]
    ...
    # 确定roi使用哪一层level的fm。 fm5为最底层, 分辨率最高
    # h, w是归一化后的值，乘上图片size，还原到原图
    ...
    for i, level in enumerate(range(2, 6)): # (0,2),(1,3)……
        ix  = roi_level==level # tensor([1, 0, 0...], dtype=torch.uint8)
        # any(): 判断给定的可迭代参数iterable是否全部为False，则返回False，如果有一个为True，则返回True。
        if not ix.any():
            continue
        # torch.nonzero(ix): tensor([[1],[3],...]) 返回一个tensor，其中是非0值的索引。
        ix = torch.nonzero(ix)[:,0] # tensor([1,3,...])00000000002
        level_boxes = boxes[ix.data, :] # 取出那一层的boxes

        # Keep track of which box is mapped to which level
        box_to_level.append(ix.data)

        # Stop gradient propogation to ROI proposals
        level_boxes = level_boxes.detach() # 不求level_boxes的参数
...
def refine_detections(rois, probs, deltas, window, config):
    ...
    for i, class_id in enumerate(unique1d(pre_nms_class_ids)):
        ...
        # Sort
        ix_rois = pre_nms_rois[ixs.data] #　提取出预测class_id类物体的roi
        ix_scores = pre_nms_scores[ixs]
        ix_scores, order = ix_scores.sort(descending=True)
        ix_rois = ix_rois[order.data,:] # 将roi根据分值排序
...
class RPN(nn.Module):
    ...
    def __init__(self, anchors_per_location, anchor_stride, depth):
        ...
        self.anchors_per_location = anchors_per_location # 每一个特征点对应的anchor数
        self.anchor_stride = anchor_stride # 每多少个特征点取一个anchor
        self.depth = depth
    ...
    #     layer_outputs.append(self.rpn(p)) //以每一层fm为单位，调用self.rpn()
    def forward(self, x):
...
#  pool_size: 处理后得到的feature map，一般为7*7
...
class Classifier(nn.Module):
    def __init__(self, depth, pool_size, image_shape, num_classes):
        ...
        self.linear_class = nn.Linear(1024, num_classes) # 全连接层，得到roi在各个类别上的分数
        self.softmax = nn.Softmax(dim=1)

        self.linear_bbox = nn.Linear(1024, num_classes * 4) # 全连接层，得到roi的位置在各个类别上的偏移量
 
    # self.classifier(mrcnn_feature_maps, rpn_rois)
    # mrcnn_feature_maps = [p2_out, p3_out, p4_out, p5_out]
    # rpn_rois: [proposal_count, 4]
    def forward(self, x, rois):
        ...
        x = x.view(-1,1024)
        mrcnn_class_logits = self.linear_class(x) # [rois, classes]
        mrcnn_probs = self.softmax(mrcnn_class_logits) # [rois, classes]

        mrcnn_bbox = self.linear_bbox(x)
        mrcnn_bbox = mrcnn_bbox.view(mrcnn_bbox.size()[0], -1, 4) # [rois, classes, 4]
...
class Mask(nn.Module):
    def __init__(self, depth, pool_size, image_shape, num_classes):
        ...
        self.conv1 = nn.Conv2d(self.depth, 256, kernel_size=3, stride=1) # 通道数改变
        # eps: 分母附加值，防止除以方差时出现除0操作
        ...
        # 得到每个特征点(原图的某个区域)在各类别的分数
        self.conv5 = nn.Conv2d(256, num_classes, kernel_size=1, stride=1) 
        self.sigmoid = nn.Sigmoid() # 将类别分数归一化，得到概率
        self.relu = nn.ReLU(inplace=True)
...
def compute_losses(rpn_match, rpn_bbox, rpn_class_logits, rpn_pred_bbox, target_class_ids, mrcnn_class_logits, target_deltas, mrcnn_bbox, target_mask, mrcnn_mask):
    ...
    rpn_class_loss = utils.compute_rpn_class_loss(rpn_match, rpn_class_logits) #　rpn二分类loss
    # target_bbox: [batch, max positive anchors, (dy, dx, log(dh), log(dw))]. Uses 0 padding to fill in unsed bbox deltas.
    # rpn_bbox: [batch, anchors, (dy, dx, log(dh), log(dw))]
    rpn_bbox_loss = compute_rpn_bbox_loss(rpn_bbox, rpn_match, rpn_pred_bbox) # rpn位置loss(仅对正样本计算)
    # target_class_ids: [batch, num_rois]. Integer class IDs.
    # pred_class_logits: [batch, num_rois, num_classes]
    mrcnn_class_loss = compute_mrcnn_class_loss(target_class_ids, mrcnn_class_logits) # roi类别loss
    # target_bbox: [batch, num_rois, (dy, dx, log(dh), log(dw))]
    # target_class_ids: [batch, num_rois]. Integer class IDs.
    # pred_bbox: [batch, num_rois, num_classes, (dy, dx, log(dh), log(dw))]
    mrcnn_bbox_loss = compute_mrcnn_bbox_loss(target_deltas, target_class_ids, mrcnn_bbox) # roi位置loss(Only positive ROIs contribute to the loss.)
    # target_masks: [batch, num_rois, height, width].
    # A float32 tensor of values 0 or 1.
    # pred_masks: [batch, proposals, height, width, num_classes]
    mrcnn_mask_loss = compute_mrcnn_mask_loss(target_mask, target_class_ids, mrcnn_mask) # 计算mask loss
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    bbox = utils.extract_bboxes(mask) # 根据每一层的mask画出最小适应框
    ...
    # mask: [height, width, instance_count] mask监督数据
...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
    ...
    #　求出对于每个anchor,　iou最大的box
    anchor_iou_argmax = np.argmax(overlaps, axis=1)
    # 求出每个anchor, 其和box最大的iou
    ...
    #　求出对于每个box,　iou最大的anchor, 将其置为正样本
...
class MaskRCNN(nn.Module):
    ...
    def set_log_dir(self, model_path=None):
        ...
        self.epoch = 0 # If None, start epochs from 0.
    ...
    def predict(self, input, mode):
        ...
        if mode == 'inference':
            ...
            # detections: [num_detections , (y1, x1, y2, x2, class_id, score)]
    ...
    def mold_inputs(self, images):
        # 保存原始pic -> pic_resize的变换信息，便于还原mask
        ...
        for image in images:
            ...
            # Build image_meta
            # compose_image_meta的4个参数: image_id, image_shape, window, active_class_ids
    ...
    # self.unmold_detections(detections[i], mrcnn_mask[i], image.shape, windows[i]) 第i张图片
    def unmold_detections(self, detections, mrcnn_mask, image_shape, window):
        ...
        full_masks = []
        for i in range(N):
            # Convert neural network mask to full size mask 将mask还原到原图
