...
class RoIAlign(nn.Module):
    ...
    def forward(self, featuremap, boxes, box_ind):
        ...
        :return: MxCxoHxoW
        """
        # torch.split(tensor,size,dim) 在dim维上拆分成若干个size大小的tensor
        x1, y1, x2, y2 = torch.split(boxes, 1, dim=1)
        image_height, image_width = featuremap.size()[2:4]

        if self.transform_fpcoor:
            # 每个物体所在区域要进行roi_align， 池化成crop_width * crop_height个区域
            ...
            # (ny0, nx0): 得到左上第一个block的中心坐标
            # (ny0 + nh, nx0 + nw): 得到右下最后一个的block的中心坐标
